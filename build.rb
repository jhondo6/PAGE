...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        #code-to-process { width: 100%; height: 200px; background: #06090f; color: #c9d1d9; border: 1px solid #30363d; border-radius: 8px; padding: 1rem; font-family: 'Courier New', Courier, monospace; font-size: 0.9rem; resize: none; outline: none; }
        #code-to-process:focus { border-color: #00ff66; box-shadow: 0 0 10px rgba(0,255,102,0.1); }
        
        .architect-controls { display: flex; justify-content: center; }
        ...
        #map-visualization { background: #06090f; border: 1px solid #30363d; border-radius: 8px; min-height: 250px; padding: 1.5rem; color: #8b949e; overflow: auto; }
...
<body>
    ...
    <section class="container" id="code-architect">
        ...
        <div class="card">
            ...
            <p>Cole um trecho de código abaixo e clique em 'Mapear'. A IA extrairá os nós de execução para montar a estrutura do diagrama.</p>
            
            <div class="architect-wrapper">
                <textarea id="code-to-process" placeholder="// Cole seu código aqui...
function calcularDesconto(preco, cupom) {
  ...
  return preco;
                ...
                <div id="map-visualization">O mapa lógico e os nós extraídos aparecerão aqui...</div>
            </div>
        </div>
    </section>
    ...
    <script type="module">
        ...
        architectBtn.addEventListener('click', async () => {
            ...
            if (!code) return alert('Por favor, cole algum código para mapear.');

            architectBtn.disabled = true;
            architectBtn.innerText = 'Processando...';
            vizArea.innerHTML = 'Analisando estrutura e gerando nós do mapa...';

            const prompt = `
                Você é o "The Code Architect". Analise o código fornecido e retorne APENAS um objeto JSON válido (sem texto adicional e sem blocos de código markdown) na seguinte estrutura:
                {
                  "moduloPrincipal": "Nome do módulo ou script",
                  "nos": [
                    { "id": 1, "nome": "Entrada de Dados", "tipo": "entrada", "descricao": "Explicacao simples" },
                    { "id": 2, "nome": "Processamento Lógico", "tipo": "condicional", "descricao": "Explicacao simples" }
                  ]
                }
                Código:
                \${code}
            ...
            try {
                ...
                vizArea.innerHTML = \`<pre style="color: #00ff66;">\${JSON.stringify(data, null, 2)}</pre>\`;
            } catch (error) {
                console.error(error);
                vizArea.innerHTML = "❌ Erro ao conectar com a API do Gemini. Verifique a chave ou tente um código mais simples.";
            } finally {
